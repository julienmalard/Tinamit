...
async def leer_arch_egr(archivo, años=None, procesar=True):
    años = [años] if isinstance(años, int) else años

    dic_datos = {}

    n_est, n_polí = await _buscar_forma(archivo)
    n_años = await _buscar_n_años(archivo) if años is None else len(años)
    async with await trio.open_file(archivo, 'r') as d:

        async for a in _avanzar_año(d, año=años):

            for e in range(1, n_est + 1):
                await _avanzar_a_estación(e, d)

                for p in range(1, n_polí + 1):
                    await _avanzar_a_polígono(p, d)
...
async def _buscar_forma(arch):
    n_est = n_polí = 0
    # para hacer: más elegante
    async with await trio.open_file(arch, 'r') as d:
        f = await d.readline()
...
async def _avanzar_año(d, año=None):
    f = await d.readline()
    if año is None:
...
async def _avanzar_a_estación(estación, d):
    f = await d.readline()
    while re.match(r'\W*Season:\W+%i\W' % estación, f) is None:
        f = await d.readline()


async def _avanzar_a_polígono(polígono, d):
    f = await d.readline()

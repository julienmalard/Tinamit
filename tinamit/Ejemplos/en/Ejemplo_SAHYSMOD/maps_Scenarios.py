...
vars_interés = {'Soil salinity Tinamit CropA': {'col': ['#00CC66', '##FFCC66', '#FF431D'],'escala_núm': [0.2, 6]},
                'Watertable depth Tinamit': {'col':['#FF6666', '#FFCC66', '#00CC66'],'escala_núm': [0.828, 4]}}
...
#for a in Escenarios:
    ...
    n_csv = [x for x in os.listdir(File_dir) if x.endswith('csv')]
    for i in n_csv:

        datos = leer_egr_mds((os.path.join(File_dir, f'{i}')), var=v)
        runs.update({i: datos})

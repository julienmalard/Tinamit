...
def analyze(problem, X, Y,
    ...
    for i in range(num_vars):
        rank_j_k.append(get_resampled_ee(
            ee[i, :], num_trajectories, num_resamples, conf_level))
    ...
    coefficient.sort()
    if fw != None:
        fw.write(json.dumps(coefficient))
        fw.flush()
...
# new added codes for Bootstrap resampling
def get_resampled_ee(ee, num_trajectories, num_resamples,
                     conf_level):
    ee_resampled = np.zeros([num_trajectories])
    mu_star_resampled = np.zeros([num_resamples])
...
# new added
def compute_rank_correlation_coefficient(ee, num_trajectories, num_resamples,
                                         conf_level):
    ee_resampled = np.zeros([num_trajectories])
    mu_star_resampled = np.zeros([num_resamples])

...
base_dir = os.path.join("C:\\Users\Azhar Inam\PycharmProjects\Tinamit\\tinamit\Ejemplos\en\Ejemplo_SAHYSMOD", 'Shape_files')
...
for v, d in vars_interés.items():
    ...
    for a in Escenarios:
        for i in range(5):
            datos = leer_egr_mds((os.path.join(File_dir, f'{a} clima{i}.csv')), var=v)
